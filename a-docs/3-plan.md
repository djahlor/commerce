...
- [x]  **Step 2: Configure Environment Variables**
    - **Task**: Create `.env.local` based on Next.js Commerce `.env.example` AND add keys needed for Polar (API Key), Supabase (URL, Anon Key, Service Role Key), Clerk (Publishable Key, Secret Key), OpenAI (API Key), Resend (API Key). Document all keys in `.env.example`.
...
- [ ]  **Step 9: Finalize Database Schemas & Migrations**
    - **Task**: Review and finalize all required Drizzle schemas (`profiles`, `purchases`, `outputs`, `raw_outputs`, `temp_carts` if using workaround) based on Tech Spec v2. Run migrations to sync the database.
...
- [ ]  **Step 10: Implement DB Actions for Purchases & Outputs**
    - **Task**: Create server actions (`actions/db/purchases-actions.ts`, `actions/db/outputs-actions.ts`) for CRUD operations on `purchases` and `outputs` tables using Drizzle and the `ActionState` pattern. Include functions like `createPurchaseAction`, `updatePurchaseStatusAction`, `createOutputAction`, `getUserOutputsAction`.
...
- [ ]  **Step 11: Implement DB Actions for Profiles**
    ...
    - **Step Dependencies**: Steps 4, 9
- [ ]  **Step 12: Implement AI & PDF Generation Actions**
    - **Task**: Set up AI client (`lib/ai.ts`), PDF library helpers (`lib/pdf.ts`). Create the core `generatePDFAction` in `actions/pdf-actions.ts` outlining the flow (AI call -> format -> save to storage -> update DB -> trigger email). Implement Supabase storage upload logic (`actions/storage/pdf-storage-actions.ts`).
    - **Files**: `lib/ai.ts`, `lib/pdf.ts`, `actions/pdf-actions.ts`, `actions/storage/pdf-storage-actions.ts`
    - **Step Dependencies**: Steps 1, 2, 10
- [ ]  **Step 13: Implement Email Action**
...
### Phase 3: Polar Integration (Steps 14-17)

*Goal: Integrate the payment and order processing backend.*

- [ ]  **Step 14: Setup Polar SDK & Products**
    ...
    - **Step Dependencies**: Steps 1, 2
- [ ]  **Step 15: Implement Polar Checkout Action**
...
- [ ]  **Step 16: Implement Polar Webhook Handler**
    ...
    - **Step Dependencies**: Steps 2, 14
    - **Mitigation**: Test signature verification thoroughly. Log incoming payloads during development.
- [ ]  **Step 17: Connect Webhook to Backend Logic**
    - **Task**: Update the webhook handler to: retrieve full cart details if using workaround, call `createPurchaseAction` to save to Supabase `purchases`, then trigger `generatePDFAction` for each relevant item (potentially via a queue). Ensure immediate 200 response to Polar after verification/trigger. Implement idempotency check.
    - **Files**: `app/api/webhooks/polar/route.ts` (update), `actions/db/purchases-actions.ts` (call), `actions/pdf-actions.ts` (trigger)
    - **Step Dependencies**: Steps 10, 12, 16

---

### Phase 4: Frontend - Cart Adaptation & Checkout Flow (Steps 18-21)

*Goal: Make the Next.js Commerce cart work with local state and Polar.*

- [ ]  **Step 18: Implement Local Cart State Management**
    ...
    - **Step Dependencies**: Step 1
- [ ]  **Step 19: Adapt Cart UI Components**
...
- [ ]  **Step 20: Adapt Cart Actions**
    ...
    - **Step Dependencies**: Step 18, 19
- [ ]  **Step 21: Connect Cart to Polar Checkout Action**
...
### Phase 5: Frontend - Custom Pages & Features (Steps 22-28)

*Goal: Build the unique pages and features of the application.*

- [ ]  **Step 22: Implement URL Input on Product Page**
    - **Task**: Create the `UrlInput` component (`components/product/url-input.tsx`) using Shadcn `Form`, `Input`, `Label` and `react-hook-form`. Integrate this component into the adapted `app/product/[handle]/page.tsx`. Ensure the entered URL is captured and passed correctly when adding the item to the local cart state (Step 20).
    ...
    - **Step Dependencies**: Steps 5, 20
- [ ]  **Step 23: Build Custom Success Page**
    - **Task**: Create `app/(auth)/success/page.tsx`. Implement the initial "Processing..." state with animation (`components/success/processing-spinner.tsx`). Add logic to check purchase status (e.g., poll Supabase based on order ID from query param) and display download links (using signed URLs from Supabase storage via server action) or the dashboard prompt upon completion.
...
- [ ]  **Step 24: Build Dashboard Structure & Layout**
...
- [ ]  **Step 25: Implement Dashboard Data Fetching & Display**
    ...
    - **Step Dependencies**: Steps 10, 23 (signed URLs), 24
- [ ]  **Step 26: Implement Dashboard Progress Tracker**
    ...
    - **Step Dependencies**: Steps 5, 25
- [ ]  **Step 27: Implement Dashboard Upsell Display & Purchase**
    ...
    - **Step Dependencies**: Steps 5, 15, 25
- [ ]  **Step 28: Customize Marketing Pages & Content**
...
### Phase 6: Authentication & Authorization (Steps 29-31)

*Goal: Secure the dashboard and link user accounts.*

- [ ]  **Step 29: Configure Clerk Authentication Routes & Middleware**
...
- [ ]  **Step 30: Implement Clerk User Linking Logic**
...
### Phase 7: Error Handling, Edge Cases & Validation (Steps 32-35)

*Goal: Make the application robust.*

- [ ]  **Step 32: Implement Robust URL Validation**
    - **Task**: Enhance URL validation (`lib/validation.ts`) to check for valid e-commerce site patterns if possible (beyond just URL format). Implement checks both client-side (in `UrlInput`) and server-side (potentially in webhook handler before triggering AI).
    - **Files**: `lib/validation.ts`, `components/product/url-input.tsx`, `app/api/webhooks/polar/route.ts`
    - **Step Dependencies**: Steps 22, 17
- [ ]  **Step 33: Implement AI/PDF Retry & Failure Handling**
    - **Task**: Add retry logic (e.g., 1-2 retries with exponential backoff) within `generatePDFAction` for transient errors from OpenAI or PDF library. On persistent failure, update `purchases.status` to 'failed' and potentially trigger a notification (email admin/user).
    - **Files**: `actions/pdf-actions.ts`
    - **Step Dependencies**: Step 12
- [ ]  **Step 34: Enhance Webhook Handler Robustness**
    - **Task**: Ensure webhook handler explicitly checks if `polarOrderId` already exists in `purchases` to prevent duplicate processing (idempotency). Add detailed logging for errors during parsing, DB writes, or PDF trigger. Consider using a queue (`lib/queue.ts` with Supabase pg_net or Vercel KV/Queues) for decoupling PDF generation if load/latency is a concern.
    - **Files**: `app/api/webhooks/polar/route.ts`, `lib/queue.ts` (optional), `lib/logger.ts` (optional setup)
    - **Step Dependencies**: Step 17
- [ ]  **Step 35: Handle Polar API Errors**
    ...
    - **Step Dependencies**: Steps 6, 15, 21

---

### Phase 8: Testing (Steps 36-39)

*Goal: Ensure application quality and reliability.*

- [ ]  **Step 36: Write Unit & Integration Tests**
    - **Task**: Write tests for critical server actions (DB actions, Polar checkout action mock, PDF generation mock), utility functions (validation), and webhook handler logic (mocking Polar event). Use Vitest/Jest.
...
- [ ]  **Step 38: Perform Manual QA & Cross-Browser Testing**
    - **Task**: Manually test all user flows on major browsers (Chrome, Firefox, Safari) and different screen sizes (desktop, tablet, mobile). Check for UI glitches, broken links, logical errors.
    - **User Instructions**: Thoroughly test the application as a real user would.
    - **Step Dependencies**: Step 37
- [ ]  **Step 39: Stress Test Critical Paths (Optional but Recommended)**
    - **Task**: Simulate high load on the Polar webhook endpoint and the AI/PDF generation pipeline to identify bottlenecks or scaling issues. Use tools like k6 or [artillery.io](http://artillery.io/).
    - **Files**: `tests/stress/*.test.js` (or similar)
    - **Step Dependencies**: Step 34, 33

---

### Phase 9: Finalization & Deployment (Steps 40-44)

*Goal: Polish, deploy, and prepare for launch.*

- [ ]  **Step 40: UI Polish & Animations**
...
- [ ]  **Step 41: Final Copywriting & Content Review**
    ...
    - **Step Dependencies**: Step 28
- [ ]  **Step 42: Deploy to Vercel Production**
    - **Task**: Configure Vercel project settings (connect Git repo). Deploy the `main` branch. Set up production environment variables in Vercel UI for all required services (Polar, Supabase, Clerk, OpenAI, Resend).
    - **Files**: Vercel Dashboard.
    - **Step Dependencies**: All previous steps.
- [ ]  **Step 43: Configure Production Webhooks & Redirects**
...
- [ ]  **Step 44: Setup Monitoring & Logging**
    - **Task**: Ensure Vercel's built-in logging and analytics are active. Consider integrating a dedicated error tracking service (Sentry, Logtail) for production issues. Monitor webhook success rates via Polar/Vercel logs.
    - **Files**: `lib/logger.ts` (optional integration), Vercel Monitoring/Logs UI.
    - **Step Dependencies**: Step 42

---

### Phase 10: Post-Launch (Steps 45-46)

*Goal: Gather feedback and iterate.*

- [ ]  **Step 45: Gather Beta User Feedback**
    - **Task**: Invite a small group of target users (10-20) to test the live application. Collect structured feedback on usability, value, bugs, and overall experience.
    - **User Instructions**: Actively solicit and analyze feedback from beta users.
    - **Step Dependencies**: Step 44
- [ ]  **Step 46: Iterate Based on Feedback**
    - **Task**: Prioritize and implement fixes and improvements based on beta feedback before a wider public launch.
